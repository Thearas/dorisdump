...
- `--parallel` 控制导出并发量，调大导出更快，调小占用资源更少，默认机器核数
...
- `--speed` 控制回放速度，作用于上面提到的「间隔时长」，比如 `--speed 0.5` 代表放慢一倍，而 `--speed 2` 代表加快一倍

  > 原理是按比例增加或减少同一客户端的相邻 SQL 的间隔时长，注意如果 SQL 本身的执行时间过长，则 `--speed` 效果不佳

- `--client-count int` 重新设置客户端数目，并且所有 SQL 都将被均衡地分散到各个客户端并行跑，**！！！设置此值无法预料回放效果！！！**

  > 默认跟日志里的客户端数一样，以达到跟线上相同的效果
...
for day in {1..31} ; do
  ...
  for hour in {0..23} ; do
      ...
      # 查看回放结果
      dorisdump diff --min-duration-diff 1s --original-sqls $sql result -Ldebug 2>&1 | tee -a "result-$day-$hour.txt"
  done
done
```
...
导出时担心消耗资源的话，可以分批执行并设置 `--parallel=1`，内存消耗最多几十兆，一般执行时间在秒级。
