...
build-darwin:
	GOOS=darwin CGO_ENABLED=1 GOEXPERIMENT=newinliner go build -o dorisdump-darwin-$(GOARCH) -a -trimpath -ldflags "-s"
	tar czf dorisdump-darwin-$(GOARCH).tar.gz dorisdump-darwin-$(GOARCH)

build-linux:
	GOOS=linux CGO_ENABLED=1 GOEXPERIMENT=newinliner go build -o dorisdump-linux-$(GOARCH) -a -trimpath -installsuffix cgo -ldflags "-s -linkmode external -extldflags -static"
